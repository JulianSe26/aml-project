...
def dicom2image(ele):
    ...
    image_path = '../../data/siim-covid19-detection/images/{}/{}.png'.format(ele.type, image_id)
    cv2.imwrite(image_path, data)

    return [PatientID, ele.StudyInstanceUID, series_id, image_id, dcm_file.SeriesNumber, dcm_file.InstanceNumber]

if __name__ == '__main__':
    os.makedirs('../../data/siim-covid19-detection/images/train', exist_ok=True)
    os.makedirs('../../data/siim-covid19-detection/images/test', exist_ok=True)

    ################ TEST ################
    meles = []
    df = pd.read_csv('../../data/siim-covid19-detection/sample_submission.csv')
    for id in np.unique(df.id.values):
        if '_study' not in id:
            continue
        StudyInstanceUID = id.replace('_study', '')
        for rdir, _, files in os.walk('../../data/siim-covid19-detection/test/{}'.format(StudyInstanceUID)):
            for file in files:
    ...
    test_df.to_csv('../../data/siim-covid19-detection/test_meta.csv', index=False)

    ################ TRAIN ################
    meles = []
    df = pd.read_csv('../../data/siim-covid19-detection/train_study_level.csv')
    for id in np.unique(df.id.values):
        StudyInstanceUID = id.replace('_study', '')
        for rdir, _, files in os.walk('../../data/siim-covid19-detection/train/{}'.format(StudyInstanceUID)):
            for file in files:
                file_path = os.path.join(rdir, file)
                filename, file_extension = os.path.splitext(file_path)
                if file_extension in ['.dcm', '.dicom']:
    ...
    train_df = pd.DataFrame(
        data=np.array(results), 
        columns=['patientid', 'studyid', 'series_id', 'imageid', 'SeriesNumber', 'InstanceNumber'])
    train_df.to_csv('../../data/siim-covid19-detection/train_meta.csv', index=False)