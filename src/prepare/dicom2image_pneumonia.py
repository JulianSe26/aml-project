...
def dicom2image(ele):
    ...
    image_path = '../../data/RSNA/images/{}/{}.png'.format(ele.type, PatientID)
...
if __name__ == '__main__':
    os.makedirs('../../data/RSNA/images/train', exist_ok=True)
    os.makedirs('../../dataset/RSNA/images/test', exist_ok=True)

    ################ TEST ################
    meles = []
    df = pd.read_csv('../../data/RSNA/stage_2_sample_submission.csv')
    for patientId in np.unique(df.patientId.values):
        dcm_path = '../../data/RSNA/stage_2_test_images/{}.dcm'.format(patientId)
    ...
    test_df.to_csv('../../data/RSNA/test_meta.csv', index=False)

    ################ TRAIN ################
    meles = []
    df = pd.read_csv('../../data/RSNA/stage_2_train_labels.csv')
    for patientId in np.unique(df.patientId.values):
        dcm_path = '../../data/RSNA/stage_2_train_images/{}.dcm'.format(patientId)
    ...
    train_df = pd.DataFrame(
        data=np.array(results), 
        columns=['studyid', 'patientid', 'imageid', 'Modality', 'PatientSex', 'BodyPartExamined', 'ImagerPixelSpacingX', 'ImagerPixelSpacingY'])
    train_df.to_csv('../../data/RSNA/train_meta.csv', index=False)